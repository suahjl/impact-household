...
cash = cash[
    ["response", "horizon_year"] + [i for i in cash.columns if (": Total" in i)]
]
cash.columns = [i.replace(": Total", "") for i in cash.columns]
scenarios_cash = [
    i for i in cash.columns if ("response" not in i) and ("horizon_year" not in i)
]
...
# Sort
net = net.sort_values(
    by=["response", "horizon_year", "Petrol Scenarios"], ascending=[True, True, True]
)

# IV --- Generate cumulative net impact (same as CIRFs)
net_cumul = (
    net.groupby(["Petrol Scenarios", "response"])[scenarios_cash].sum().reset_index()
)
net_cumul = net_cumul.sort_values(
    by=["response", "Petrol Scenarios"], ascending=[True, True]
)
...
for response, ltavg in zip(choices_macro_response, list(dict_ltavg.values())):
    for horizon in list(net["horizon_year"].unique()):
        # Subset + deep copy
        net_sub = net.loc[
            (net["response"] == response) & (net["horizon_year"] == horizon),
            ["Petrol Scenarios"] + scenarios_cash,
        ].copy()
        # Set index
...
pil_img2pdf(list_images=list_file_names, extension="png", pdf_name=file_pdf)
telsendfiles(conf=tel_config, path=file_pdf + ".pdf", cap=file_pdf)
...
for response, ltavg in zip(choices_macro_response, list(dict_ltavg.values())):
    for horizon in list(net["horizon_year"].unique()):
        ...
        net_sub_disp = (
            pd.DataFrame(net_sub.min(axis=0)).round(2).transpose().astype("str")
            + "\n to \n"
...
for response, ltavg in zip(choices_macro_response, list(dict_ltavg.values())):
    # Subset + deep copy
    net_sub = net_cumul.loc[
        net_cumul["response"] == response, ["Petrol Scenarios"] + scenarios_cash
    ].copy()
    # Set index
    net_sub = net_sub.set_index("Petrol Scenarios")
    # Generate heatmap
