...
# Base columns
cols_all_endog = ['cpielec', 'pc', 'gfcf', 'gdp', 'neer', 'cpi', 'mgs10y']  # same as suah (2022) + cpielec
...
for endog in tqdm(cols_all_endog_perm):
    ...
    cond_elec_costpush = bool(
        (
                tab_irf_a0.loc[
                    (tab_irf_a0['shock'] == 'cpielec') & (tab_irf_a0['response'] == 'gdp')
                    , 'irf'].iloc[0] < 0
        ) and
        (
                tab_irf_a0.loc[
                    (tab_irf_a0['shock'] == 'cpielec') & (tab_irf_a0['response'] == 'pc')
                    , 'irf'].iloc[0] < 0
        ) and
        (
                tab_irf_a0.loc[
                    (tab_irf_a0['shock'] == 'cpielec') & (tab_irf_a0['response'] == 'gfcf')
                    , 'irf'].iloc[0] < 0
        ) and
        (
                tab_irf_a0.loc[
                    (tab_irf_a0['shock'] == 'cpielec') & (tab_irf_a0['response'] == 'cpi')
                    , 'irf'].iloc[0] > 0
        )
    )
...
# Generate output
tab_irf_all.to_parquet(path_output + 'macro_var_elec_irf_all_varyx_kitchensink_' + input_suffix + '.parquet')
tab_irf_all.to_csv(path_output + 'macro_var_elec_irf_all_varyx_kitchensink_' + input_suffix + '.csv', index=False)
tab_irf_all_avg.to_parquet(path_output + 'macro_var_elec_irf_all_varyx_kitchensink_avg_' + input_suffix + '.parquet')
tab_irf_all_avg.to_csv(path_output + 'macro_var_elec_irf_all_varyx_kitchensink_avg_' + input_suffix + '.csv',
                       index=False)
tab_irf_narrative.to_parquet(path_output + 'macro_var_elec_irf_all_varyx_narrative_' + input_suffix + '.parquet')
tab_irf_narrative.to_csv(path_output + 'macro_var_elec_irf_all_varyx_narrative_' + input_suffix + '.csv', index=False)
tab_irf_narrative_avg.to_parquet(
    path_output + 'macro_var_elec_irf_all_varyx_narrative_avg_' + input_suffix + '.parquet')
tab_irf_narrative_avg.to_csv(path_output + 'macro_var_elec_irf_all_varyx_narrative_avg_' + input_suffix + '.csv',
                             index=False)
tab_irf_narrative2.to_parquet(path_output + 'macro_var_elec_irf_all_varyx_narrative2_' + input_suffix + '.parquet')
tab_irf_narrative2.to_csv(path_output + 'macro_var_elec_irf_all_varyx_narrative2_' + input_suffix + '.csv',
                          index=False)
tab_irf_narrative2_avg.to_parquet(
    path_output + 'macro_var_elec_irf_all_varyx_narrative2_avg_' + input_suffix + '.parquet')
tab_irf_narrative2_avg.to_csv(path_output + 'macro_var_elec_irf_all_varyx_narrative2_avg_' + input_suffix + '.csv',
                              index=False)
# Count number of cholesky ordering that satisfy the narrative sign restrictions
print('\n----- Number of Cholesky orders that satisfy the specified narrative sign restrictions (set 1): '
      + str(len(list_endog_narrative_fit)) + ' -----')

print('\n----- Number of Cholesky orders that satisfy the specified narrative sign restrictions (set 2): '
...
fig_kitchensink_avg = manual_irf_subplots(
    ...
    irf_col='irf',
    horizon_col='horizon',
    main_title='All Possible Cholesky Orders (With Electricity CPI): Average IRFs' + ' (' + str(
...
fig_narrative_avg = manual_irf_subplots(
    ...
    irf_col='irf',
    horizon_col='horizon',
    main_title='All Cholesky Orders Satisfying Narrative Sign Restrictions (With Electricity CPI): Average IRFs'
...
fig_narrative2_avg = manual_irf_subplots(
    ...
    irf_col='irf',
    horizon_col='horizon',
    main_title='All Cholesky Orders Satisfying Narrative Sign Restrictions 2 (With Electricity CPI): Average IRFs'
