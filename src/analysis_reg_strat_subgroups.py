...
def load_clean_estimate(
    ...
    # I --- Load data
    if use_balanced:
        df = pd.read_parquet(path_subgroup + 'hies_consol_agg_balanced_' + full_input_suffix + '.parquet')
    elif not use_balanced:
        df = pd.read_parquet(path_subgroup + 'hies_consol_agg_' + full_input_suffix + '.parquet')
...
round = 1
for incgroup_suffix, incgroup_nice in tqdm(zip(list_incgroups_suffix, list_incgroups_nice)):
    for benefit_objective, benefit_objective_nice, truncate_threshold in zip(list_benefit_objectives,
                                                                             list_benefit_objectives_nice,
                                                                             list_truncate_thresholds):
        # Input file suffixes
        for n_obj in range(1, truncate_threshold + 1):
            if n_obj < truncate_threshold:
                file_suffix = 'mean_' + incgroup_suffix + '_' + benefit_objective + '_' + str(n_obj) + hhbasis_suffix
                n_obj_nice = str(n_obj)
            elif n_obj == truncate_threshold:
                file_suffix = 'mean_' + incgroup_suffix + '_' + benefit_objective + '_' + \
                              str(n_obj) + 'plus' + hhbasis_suffix
                n_obj_nice = str(n_obj) + '+'
            elif n_obj > truncate_threshold:
            ...
            heatmap_params_table_fe = heatmap(
                ...
                outputfile='./output/params_table_fe_strat_cons_' +
                           income_choice + '_' + fd_suffix + '_' + file_suffix + '.png',
            ...
            list_filenames_fe = list_filenames_fe + ['./output/params_table_fe_strat_cons_' +
                                                     income_choice + '_' + fd_suffix + '_' + file_suffix]
...
# Sort subgroups
if show_ci:
    col_sort_order = ['method', 'gross_income_group', 'benefit_objective', 'benefit_recipients',
                      'Parameter', 'LowerCI', 'UpperCI']
if not show_ci:
    col_sort_order = ['method', 'gross_income_group', 'benefit_objective', 'benefit_recipients', 'Parameter']
