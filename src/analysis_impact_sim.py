...
for tiering, tiering_nice in zip(['tiered', 'flat'], ['Tiered', 'Flat']):
    for restrict_threshold in tqdm([3, 4, 5, False]):
        ...
        landing_rpc = 100 * ((landing * 1000000000 / (pc_deflator_2023 / 100)) / rpc_2022)

        # Convert into real GDP growth
        landing_rgdp = 100 * ((landing * 1000000000 / (deflator_2023 / 100)) / rgdp_2022)
...
def repeated_aggregate_impact(aggregate, landing_shock_rpc, landing_shock_rgdp, indirect, rounds_to_repeat):
    # Deep copy
    ...
    repeated_agg.loc[
        ...
        'landing_impact'] = landing_shock_rpc
    repeated_agg.loc[
        (
                (repeated_agg['horizon_year'] <= rounds_to_repeat) &
                (repeated_agg['shock'] == 'Private Consumption') &
        ...
        'landing_impact'] = landing_shock_rgdp
...
for tiering, tiering_nice in \
    ...
    for restrict_threshold, restrict_threshold_nice in \
        ...
        if landing_impact_tracker == 0:
            ...
            allcombos = allcombos.merge(repeated_agg, how='left', on=['shock', 'response', 'horizon_year'])
