...
for year, year_nice in zip(list_years, list_years_nice):
    for quantile, quantile_nice in tqdm(zip(list_quantiles, list_quantiles_nice)):
        ...
        # generate tables
        dfi.export(d,
                   './output/tab_inc_growth_change_' + 'gross_income' + '_' + str(year) + '_' + quantile_nice + '.png')
        list_files_tab = list_files_tab + \
                         ['./output/tab_inc_growth_change_' + 'gross_income' + '_' + str(year) + '_' + quantile_nice]
# Save as single pdf
pil_img2pdf(
    list_images=list_files_tab,
    extension='png',
    pdf_name='./output/tab_inc_growth_change_' + 'gross_income' + '_' + 'year_quantile'
)
telsendfiles(
    conf=tel_config,
    path='./output/tab_inc_growth_change_' + 'gross_income' + '_' + 'year_quantile' + '.pdf',
    cap='tab_inc_growth_change_' + 'gross_income' + '_' + 'year_quantile'
)
